...
1.  添加以下代码以导入从 NuGet 导入的 Azure. .Queues 包中包含的 Azure.Messaging.ServiceBus 命名空间 ：
...
1.  通过将“serviceBusConnectionString”字符串常量的值设置为你之前在本实验室中记录的“服务总线命名空间”的“主连接字符串”来更新该字符串常量 。
...
1.  查看 Program.cs文件，它现在应该包含以下代码。 请注意，`<serviceBus-connection-string>` 占位符表示目标 Azure 服务总线命名空间的连接字符串：
...
1.  选择“Service Bus Explorer (预览版)”边栏选项卡。
...
1.  和之前的操作一样，通过将“serviceBusConnectionString”字符串常量的值设置为你之前在本实验室中记录的“服务总线命名空间”的“主连接字符串”来更新该字符串常量  。
...
1.  查看 Program.cs文件，它现在应该包含以下代码。 `<serviceBus-connection-string>` 占位符表示目标 Azure 服务总线命名空间的连接字符串：
