...
1. 在“应用服务概述”窗格的“Essentials”中，记录 URL 链接的值。   你将在稍后的实验室中使用此值。
...
1. 在门户中的“Cloud Shell”命令提示符处，执行以下操作：
    ...
    a.  输入以下命令并选择“Enter”以获取 Azure CLI 根级别的子组和命令列表：

    ```bash
    az --help
    ```

    b.  输入以下命令，然后选择“Enter”以获取资源提供程序可用的命令列表：

    ```bash
    az provider --help
    ```

    c.  输入以下命令，然后按“Enter”键以列出所有当前注册的提供程序：

     ```bash
     az provider list
     ```

    d.  输入以下命令，然后按 Enter 键以仅列出当前注册的提供程序的命名空间：

     ```bash
     az provider list --query "[?registrationState=='Registered'].namespace"
     ```

    e.  观察当前注册的提供程序列表。 Microsoft.CDN 提供商当前未在提供商列表中。

    f.  输入以下命令，然后按 Enter 以获取注册新提供商所需的标记：

     ```bash
     az provider register --help
     ```

    g.  输入以下命令，然后按 Enter（使用当前订阅）注册 Microsoft.CDN 命名空间：
...
1. 在“添加终结点”弹出对话框中，执行以下操作，然后选择“添加”： 
    ...
    | “源类型”下拉列表            | 选择“存储” |
    | “源主机名”下拉列表        | 为之前在本实验中创建的存储帐户选择“contenthost[yourname].blob.core.windows.net”选项。 |
    | “源路径”文本框 | 输入“/media” |
    | “源主机头”文本框 | 保留默认值。 |
    | “协议”和“源端口”部分 | 保留默认值。 |
...
1. 在“添加终结点”弹出对话框中，执行以下操作，然后选择“添加”： 
    ...
    | “源类型”下拉列表 | 选择“Web 应用” |
    | “源主机名”下拉列表 | 为之前在此实验中创建的 Web 应用选择“landingpage[yourname].azurewebsites.net”选项。 |
    | “源路径”文本框 | 保留默认值 |
    | “源主机头”文本框 | 保留默认值 |
    | “协议”和“源端口”部分 | 保留默认值 |
    | “已优化”下拉列表 | 选择“常规 Web 分发” |
...
1. 在“添加/编辑应用程序设置”弹出窗口中，配置以下设置，然后选择“确定”。 
    ...
    | “值”文本框                    | 输入你之前在本实验中记录的“contenthost[yourname]”存储帐户中的“media”容器的“URI”值。   |
    | “部署槽设置”复选框 | 不要选中。                                               |
...
1. 在“添加/编辑应用程序设置”弹出窗口中，配置以下设置，然后选择“确定”。 
    ...
    | “值”文本框                    | 输入你之前在本实验中记录的“contenthost[yourname]”存储帐户中的“video”容器的“URI”值。   |
    | “部署槽设置”复选框 | 不要选中。                                               |
