...
最后，你将设置 Cosmos DB 实例的一致性级别，并实现一个用于更改源通知的 Azure 函数。
...
1. 在“选择 API 选项”**** 边栏选项卡上，选择“Azure Cosmos DB for NoSQL”**** 框中的“创建”****。
...
1. 单击“共享访问令牌”****，然后通过单击“生成 SAS 令牌和 URL”**** 使用默认设置生成访问令牌。 记下“Blob SAS 令牌”**** 的值，因为稍后需要用到。
...
1. 在终端提示符下，运行以下命令，将 Azure Cosmos DB .NET 客户端库添加到当前打开的项目：
   ...
   > **备注**：在继续执行以下命令之前，请确保当前位于此路径下：(F):\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Upload\\****
...
1. 更新 BlobContainerUrl**** 和 BlobSASToken**** 属性，方法是对应地将它们的值分别设置为名称是“images”**** 的 Azure 存储 Blob 容器的 **URL** 属性值，和之前在此实验室中记录的 Bolb SAS 令牌值。

1. 保存 appsettings.json 文件并将其关闭****。

   > **注意**：如果收到文件为只读的提示，请选择“覆盖”。

1. 在“Visual Studio Code”**** 窗口中，右键单击“资源管理器”窗格中的 AdventureWorks.Context**** 文件夹，然后选择“在集成终端中打开”****。
...
1. 在终端提示符下，验证当前目录是否设置为“AdventureWorks.Context”（如果不是，将其更改为该目录），然后运行以下命令从 NuGet 导入 Microsoft.Azure.Cosmos ：
   ...
   > **备注**：在继续执行以下命令之前，请确保当前位于此路径下：(F):\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Context\\****
...
1. 在 AdventureWorksCosmosContext.cs**** 文件的代码编辑器选项卡中，添加以下代码：
...
1. 在终端提示符下，运行以下命令以运行 ASP.NET Web 应用程序：
   ...
   > **备注**：在继续执行以下命令之前，请确保当前位于此路径下：(F):\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Web\\****
