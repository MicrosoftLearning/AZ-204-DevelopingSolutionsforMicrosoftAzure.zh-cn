...
1. 在“选择 API 选项”**** 边栏选项卡上的“Azure Cosmos DB for NoSQL”**** 框中选择“创建”****。
...
1. 下一步，单击“共享访问令牌”****，然后单击“生成 SAS 令牌和 URL”****，以生成具有默认设置的访问令牌。 记下 Blob SAS 令牌**** 的值，因为稍后将需要用到它。
...
1. 更新 BlobContainerUrl**** 和 BlobSASToken**** 属性，方法是：将它们的值分别设置为名为 **images** 的 Azure 存储 Blob 容器的 **URL** 属性，以及此前在本实验室中记录的 Blob SAS 令牌值。
