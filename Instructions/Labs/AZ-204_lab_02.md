...
1. 在“托管”选项卡上，执行以下操作，然后选择“查看 + 创建” ：
    ...
    以下屏幕截图显示了“创建函数应用”边栏选项卡上“托管”选项卡上配置的设置 。

    ![显示“创建函数应用”边栏选项卡上“托管”选项卡上配置的设置的屏幕截图 ](./media/l02_create_a_function_app_hosting_tab.png)
...
1. 通过将 AzureWebJobsStorage 的值设置为你之前在本实验室中记录的存储帐户的连接字符串来更新该值 。
...
1. 收到打开的命令提示符时，输入以下命令，以使用 Azure Functions Core Tools 通过计时器触发器模板新建一个名为 Recurring 的函数  ：
...
1. 收到打开的命令提示符时，输入以下命令，以使用 Azure Functions Core Tools 通过 HTTP 触发器模板新建一个名为 GetSettingInfo 的函数  ：
...
#### <a name="task-3-register-azure-storage-blob-extensions"></a>任务 3：注册 Azure 存储 Blob 扩展
...
1. 收到打开的命令提示符时，运行以下命令以注册 [Microsoft.Azure.WebJobs.Extensions.Storage](https://www.nuget.org/packages/Microsoft.Azure.WebJobs.Extensions.Storage/4.0.4) 扩展：
...
1. #### <a name="task-4-write-http-triggered-and-blob-inputted-function-code"></a>任务 4：编写 HTTP 触发且 Blob 输入的函数代码
...
1. 收到打开的命令提示符时，运行以下命令以登录到 Azure 命令行接口 (CLI)：
...
1. 在显示的弹出窗口中，执行以下操作： <!--Conan, as there is only sub step here, can we just combine the two as follows: In the popup dialog that appears, in the **HTTP method** list, select **GET**.-->
    - 在“HTTP 方法”列表中，选择“GET” 。

1. 选择“运行”以测试函数。

1. 观察运行测试的结果。 JSON 内容现应包含以下代码：
