...
在本实验室中，你将在 Microsoft Entra ID 中注册应用程序，添加用户，然后测试该用户对该应用程序的访问权限，验证 Entra ID 是否可保护该应用。 你还将使用 Graph 浏览器工具生成针对 Entra ID 用户帐户的图形 API 的请求并对其进行测试。
...
1. 在“Microsoft Entra ID”**</bpt>"> **** 边栏选项卡上，选择“管理”**** 部分中的“应用注册”****。
...
1. 运行以下命令，根据模型视图控制器 (MVC) 模板创建新的 .NET Core Web 应用（将占位符 `<application_ID>`、`<tenant_ID>` 和 `<domain_Name>` 替换为之前在本实验中记录的相应值）：
    ...
    > **备注**：如果在终端中收到错误，则可能使用的是 PowerShell。 在这种情况下，请删除 `/S /Q` 标志并重新运行命令。
